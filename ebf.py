    ...
    def addPathToTree(node,path_list):
        ...
        for path1 in path_list:
            if path1.count(b'/') == 0:
                ...
                x=path1.split(b'/',1)
                if not (x[0]+b'/' in node['dirs']):
                    node['dirs'][x[0]+b'/']=_EbfUtils.createPathNode(node['name']+x[0]+b'/')
                _EbfUtils.addPathToTree(node['dirs'][x[0]+b'/'],[x[1]])    
...
#        pos = fp1.tell()
    ...
    def create(self, tagname, data, dataunit, sdef):
        ...
        """
        tagname=tobytes(tagname)
        dataunit=tobytes(dataunit)        
        sdef=tobytes(sdef)
...
class _EbfTable(object):
    ...
    def __expand(self,keyno,keysize):
        ...
        ebfh.write(fp1)
        fp1.close()
        keys1.append(b'/.tr/.ebf/htable.t'+tobytes(str(i)))
        values1.append(loc)
...
#        numpy.warnings.simplefilter("default",RuntimeWarning)
    ...
    def __create(filename,capacity,option):
        ...
        ebfh=_EbfHeader()
        ebfh.read(fp1)
        if (ebfh.name != b'/.ebf/info') or (ebfh.elements() < 4) or (ebfh.datatype !=3):        
            fp1.close()
    ...
    def put(filename,key,value):
        ...
        i=0    
        for key1 in key:
            key[i]=tobytes(key[i].strip().lower())        
            i=i+1
...
def getHeader(filename, dataname):
   ...
   >>> ebf.getHeader("check.ebf","/x")
    ...
    location = _EbfTable.get(filename, dataname)
    if location < 0:
        raise RuntimeError("Ebf error: Data object "+tostr(dataname)+" not found")
    if location >= 0:
        fp1 = open(filename, 'rb')
        header = _EbfHeader()
        fp1.seek(location, 0)
...
#        taglist=['ver-2 ','<sdef>','anonymous 8 1 1 '+str(len(descr))]
        taglist=[b'ver-2 ',b'<sdef>',b'anonymous,8,1,1,'+tobytes(str(len(descr)))]
        status=1
    else:
        taglist=[]
...
#            tag=temp[0]+' 8 '+str(len(shape))+' '+' '.join(map(str, shape))+' '+str(len(temp[1]))+' '+str(units1[i])
            tag=tobytes(temp[0])+b','+tobytes(','.join(map(str, [8,len(shape)]+shape+[len(temp[1])])))
#            tags=tag.split()
#            tag=' '.join(tags)
            taglist.append(tag)
            ...
            if len(units1[i]) >0:
                ...
                units2=b''
            tag=tobytes(temp[0])+b','+tobytes(','.join(map(str, [type1,len(shape)]+shape+[0])))+units2
...
def __sdef2descrv2(sdef,begin=0,nsize=1):
    ...
    if begin == 0:
        sdef=sdef.split(b'\n')
        begin=0
        while sdef[begin].split()[0] != b'<sdef>':
            begin=begin+1
        words=[x.strip() for x in sdef[begin+1].split(b',')]
        rank=int(words[2])
    ...
    i=0
    while i < nsize:
        words=[x.strip() for x in sdef[begin].split(b',')]
#        words=sdef[begin].split()
        ...
        if len(words) > 4+rank:
#            units1=words[4+rank]
            units1=b','.join(words[4+rank:len(words)])
        else:
            units1=b'NULL'
        begin=begin+1
        if int(words[1]) == 8:
            [dtv,units1,begin]=__sdef2descrv2(sdef,begin,n_fields)
            dt.append((tostr(words[0]),dtv,tuple(shape)))
            units.append(units1)
        elif int(words[1]) == 1:
            dt.append((tostr(words[0]),'S'+str(shape[-1]),tuple(shape[0:-1])))            
            units.append(units1)
        else:
            dt.append((tostr(words[0]),_TypeManager.itos_l(int(words[1])),tuple(shape)))
            units.append(units1)
        i=i+1
    if status ==1:
        if sdef[begin].split()[0] != b'</sdef>':
    ...
    while i < len(wl):
        ...
        if (datatype == 'S1'):
            if dims[rank-1] > 1:
                ...
                rank=rank-1
        if rank > 0:        
            dth.append((tostr(name),datatype, tuple(dims)))
        else:
            dth.append((tostr(name),datatype))
                ...
                for key in node['files']:
                    if rows > -1:
                        ...
                        if len(temp) == 0:
                            temp=numpy.array([1],dtype='int64')
                        if temp[0] == rows:
                            mydict[tostr(key)] = read(filename,node['name']+key,recon,0,begin,end)                            
                    else:
                        mydict[tostr(key)] = read(filename,node['name']+key,recon,0)
            if (recon > 0)&(len(node['dirs']) > 0):
                if recon == 2:
                    for key in list(node['dirs'].keys()):
                        mydict[tostr(key).strip('/')] = read(filename,node['name']+key,recon,0)                                    
                if recon == 1:
                    for key in list(node['dirs'].keys()):
                        if (key.startswith(b'.ebf/') == False)and(key.startswith(b'.tr/') == False):
                            mydict[tostr(key).strip('/')] = read(filename,node['name']+key,recon,0)                                    
        
        
    if len(mydict) == 0:
        print(filename+":"+tostr(path))
...
#        raise RuntimeError('Data to be written must be of type nummpy.ndarray or numpy.void or dict')
    ...
    if tagname.endswith(b'/'):
        if type(data) is dict:
            
            mykeys=list(data.keys())
            if type(dataunit) == bytes:
                dataunitl=[]
                for name in mykeys:
            ...
            for name in list(data.keys()):
                if (type(data[name]) is dict)|(type(data[name]) is numpy.void):
                    write(filename, tagname+tobytes(name)+b'/', data[name], mode, dataunitl[i])
                else:
                    write(filename, tagname+tobytes(name), data[name], mode, dataunitl[i])
                i=i+1
            ...
            if(data.size >= 1):
                ...
                for name in data1.dtype.names:
                    if data1[name].dtype.char == 'V':
                        write(filename, tagname+tobytes(name)+b'/', data1[name], mode, dataunitl[i])
                    else:
                        write(filename, tagname+tobytes(name), data1[name], mode, dataunitl[i])
                    i=i+1
            else:
        ...
        if mode == 'u' :
            if location >= 0 :
                ...
                if (header1.get_dtype() != header.get_dtype()):
                    fp1.close()
                    raise RuntimeError(b'Data to be updated not present '+tagname)                                                     
            ...
            if location >= 0 :
                ...
                if (header1.datatype != header.datatype)|(header1.datasize != header.datasize)|(header1.dim.size != header.dim.size):
                    ...
                    raise RuntimeError(b'Data to be updated not present or of mismatch size'+tagname)                                                     
                if header1.flagswap == 1:
                    fp1.close()
                    raise RuntimeError(b'Data is of different endian format '+tagname)                                                     
                fp1.seek(0, 2)
                locend=fp1.tell()            
                fp1.seek(location, 0)            
            ...
            if location >= 0 :
                fp1.close()
                raise RuntimeError(b'Data to be written already present '+tagname)
            location=fp1.tell()
            header.write(fp1)        
...
#        print 'Reading Key vals HT', 1e3*1e-3/(time.time()-start), ' Kops' ,(time.time()-start),len(keys)
    ...
    def test_header256(self):
        ...
        ebfdir='data/'
        print('ispath=',os.path.isfile(ebfdir+'header256.ebf'))
...
#        info('check_table_swap.ebf')
    ...
    def testdataunit(self):
        ...
        self.assertEqual(b"100 m/s", unit('check.ebf', '/data'))
...
#        ebfdir='/home/sharma/sw/share/ebf/'
    ...
    def testcorrectness1(self):
        ...
        data = {}
        keys = ["x1", "x2", "x3", "x4", "x5", "x6", "x9", "x10", "x11", "x12", "x13"]
        data["x1"]  =  numpy.array([b'EBF',b'EBFGH'])
        data["x2"] = numpy.arange(-65636, -65636 - 128, -1, dtype = "int32")
        data["x3"] = numpy.arange(-4294967296, -4294967296 - 128, -1, dtype = "int64")
...
#        write('/home/sharma/ebf_demo/test1.ebf', '/2d/', data, 'a',"kpc")    
        ...
        data5['x2'][:][:] = data["x2"][:][:]
        data5['x3'][:][:] = data["x3"][:][:]
#        print('point2 ',type(data5['point2'][0]))   
#        print('data4 ',data4[0].shape)   
        data5['point2'][0] = data4.copy()
...
#        info("check.ebf",1)
        ...
        write("check.ebf", "/data", data, "w",dataunit='')
#        info("check.ebf",1)
        units1=[b'u=km/s,l=\\alpha',b'u=m/s,l=\\gamma']
